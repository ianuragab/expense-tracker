...
const TransactionList = ({ data, title, loading, emptyListMessage }: TransactionListType) => {
  ...
  const handleClick = (item: TransactionType) => {
    router.push({ pathname: '/(modals)/transactionModal', params: { id: item?.id, type: item?.type, amount: item?.amount?.toString(), category: item?.category, date: (item?.date as Timestamp)?.toDate()?.toISOString(), description: item?.description, image: item?.image, uid: item?.uid, walletId: item?.walletId } })
  }
  return (
...
const TransactionItem = ({ item, index, handleClick }: TransactionItemProps) => {
  ...
  const date = (item?.date as Timestamp)?.toDate()?.toLocaleDateString('en-US', { day: 'numeric', month: 'short' });
    ...
    <Animated.View entering={FadeInDown.delay(index * 70).damping(12)} >
      <TouchableOpacity style={styles.row} onPress={() => handleClick(item)}>
        ...
        <View style={styles.amountDate}>
          <Typo size={16} fontWeight={500} color={item?.type == 'income' ? colors.primary : colors.rose}>{`${item?.type == 'income' ? '+ ₹' : '- ₹'}${item?.amount}`}</Typo>
